...
def data_cleaning(db):
    ...
    credit_score=db["Credit Score"]
    credit_score=credit_score.fillna(credit_score.median())
    kmeans=KMeans(n_clusters=2,n_init=12,random_state=1)
    c= np.column_stack((db["Credit Default"].values, credit_score.values))
    db["Cluster"]=kmeans.fit_predict(c)
    db["Credit Score"]=credit_score

    current_loan = db["Current Loan Amount"]
    current_loan = current_loan.fillna(current_loan.median())
    kmeans = KMeans(n_clusters=2, n_init=12, random_state=1)
    d = np.column_stack((db["Current Loan Amount"].values,db["Credit Default"].values))
    db["Cluster1"] = kmeans.fit_predict(d)
    db["Current Loan Amount"] = current_loan

    db["Term"] = db["Term"].map({"Short Term": 1, "Long Term": 0})
    e=np.column_s
    tack((db["Term"].values,db["Credit Default"]))
    db["Cluster2"] = kmeans.fit_predict(e)
